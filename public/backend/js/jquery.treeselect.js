...
(function ($) {
    jQuery.fn.moreorless = function (min_height, more_text, less_text) {
        ...
        this.each(function () {
            ...
            // Create the link.
            if (!this.link) {
                this.link = $(document.createElement("div")).css({
                    cursor: "pointer",
                });
            ...
            this.element.unbind("resize").bind(
                "resize",
                (function (widget) {
                    return function () {
                        clearTimeout(resizeTimer);
    ...
    $.fn.treeselect = function (params) {
        ...
        var TreeNode = function (nodeparams, root) {
            ...
            $.extend(
                this,
                {
                    ...
                    exclude:
                        {} /** An array of nodes to exclude for selection. */,
                },
                nodeparams
            );
        ...
        TreeNode.prototype.setBusy = function (state, type) {
            ...
            if (state != this.span.hasClass(type)) {
                ...
                if (state) {
                    ...
                    var othertype =
                        type == busyloading ? busyselecting : busyloading;
        ...
        TreeNode.prototype.selectDefaults = function (defaults, done) {
            ...
            while (defaultsLeft > 0 && defaultsQueue.length > 0) {
                ...
                // include defaults.
                if (
                    defaults.hasOwnProperty(queueItem.value + "-include-below")
                ) {
        ...
        TreeNode.prototype.select = function (state) {
            ...
            if (!this.input.hasClass("treenode-no-select")) {
                ...
                if (
                    state ||
                    !this.root ||
                    (this.showRoot && this.has_children)
                ) {
                    // Set the checked state.
        ...
        TreeNode.prototype.build_input = function (left) {
            ...
            if (params.inputName) {
                ...
                if (
                    typeof this.exclude[this.id] !== "undefined" ||
                    (params.inputName == "group_finder" &&
                        !this.data.roles_enabled)
                ) {
                ...
                if (params.searcher) {
                    ...
                    params.searcher(this, text, function (nodes, getNode) {
                        ...
                        for (var id in nodes) {
                            ...
                            var treenode = new TreeNode(
                                getNode ? getNode(nodes[id]) : nodes[id]
                            );
                    ...
                    this.loadAll(
                        ...
                        function (node) {
                            ...
                            if (
                                !node.root &&
                                node.title.toLowerCase().search(text) !== -1
                            ) {
        ...
        return $(this).each(function () {
            ...
            if (selectAll !== false && !root.showRoot) {
                ...
                if (
                    default_value.hasOwnProperty(root.value + "-include-below")
                ) {
                    checked = true;
                }
            ...
            var initBusy = $(document.createElement("span")).addClass(
                "treebusy"
            );
...
(function ($) {
    ...
    $.fn.chosentree = function (params) {
        ...
        params = $.extend(
            {
                inputId:
                    "chosentree-select" /** The input element ID and NAME. */,
                label: "" /** The label to add to the input. */,
                description: "" /** The description for the input. */,
                input_placeholder:
                    "Select Item" /** The input placeholder text. */,
                ...
                no_results_text:
                    "No results found" /** Shown when no results. */,
                            ...
                            if (root) {
                                ...
                                root.search(
                                    ...
                                    function (nodes, searchResults) {
                                        ...
                                        if (searchResults) {
                                            root.childlist.addClass(
                                                "chzntree-search-results"
                                            );
                                        } else {
                                            root.childlist.removeClass(
                                                "chzntree-search-results"
                                            );
                                        }
                ...
                return function (node, direct) {
                    ...
                    if (direct) {
                        ...
                        for (var id in selectedNodes) {
                            ...
                            if (
                                !node.root ||
                                (node.showRoot && node.has_children)
                            ) {
                            ...
                            if (choices) {
                                choices.prepend(
                                    choice.append(span).append(close)
                                );
                            }
                        }
                        ...
                        if (choices) {
                            ...
                            if (jQuery.fn.moreorless) {
                                ...
                                // Add this to the choices.
                                var more_text = params.more_text.replace(
                                    "%num%",
                                    numNodes
                                );
                                choices.moreorless(
