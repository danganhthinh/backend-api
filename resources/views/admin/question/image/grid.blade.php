            ...
            <table class="table table-bordered table-striped" style="table-layout: fixed">
                <thead class="table-head">
                    <tr class="sticky-thead">
                        ...
                        <th class="text-center" style="width: 14%;">画像</th>
                        <th class="text-center" style="width: 15%;">問題</th>
                    ...
                    @foreach ($question as $ques)
                        @if (isset($ques->id))
                            <tr style="height: 35px">
                                ...
                                <td style="text-align:left; padding-left:4%"  class="question-title-ellipsis">{{ basename($ques->media) }}</td>
                                ...
                                @if (in_array(Auth::user()->role_id, [\App\Consts::ADMIN]))
                                    <td>
                                        <a class="warning mr-1" data-id="{{ $ques->id }}"
                                            id="btn-edit-question"><img width=16
                                                src="/backend/images/icons/edit.png"></a>
                                        <a hidden class="danger delete" data-id="{{ $ques->id }}"
                                            id="btn-delete-question"><img width=15 style="padding-bottom: 2%;"
                                                src="/backend/images/icons/delete.png"></a>
                                    </td>
                                @endif
                                {{-- preparing datas for editting --}}
                                <textarea data-id="{{ $ques->id }}" name="title" hidden>{{ $ques->title }}</textarea>
                                <input data-id="{{ $ques->id }}" name="answer1" value="{{ $ques->answer1 }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="answer2" value="{{ $ques->answer2 }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="answer3" value="{{ $ques->answer3 }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="answer4" value="{{ $ques->answer4 }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="category_question"
                                    value="{{ $ques->category_question }}" hidden>
                                <input data-id="{{ $ques->id }}" name="correct_answer"
                                    value="{{ $ques->correct_answer }}" hidden>
                                <input data-id="{{ $ques->id }}" name="question_level"
                                    value="{{ $ques->question_level }}" hidden>
                                <input data-id="{{ $ques->id }}" name="account_name"
                                    @if ($ques->mentor) value="{{ $ques->mentor->full_name }}" @endif
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="account_id"
                                    @if ($ques->mentor) value="{{ $ques->mentor->id }}" @endif hidden>
                                <input data-id="{{ $ques->id }}" name="subject_id" value="{{ $ques->subject_id }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="status" value="{{ $ques->status }}"
                                    hidden>
                                <input data-id="{{ $ques->id }}" name="media"
                                    value="{{ basename($ques->media) }}" hidden>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
